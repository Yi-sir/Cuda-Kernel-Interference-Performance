...
@triton.jit
def grouped_matmul_tma_no_transpose_kernel(
    group_a_ptrs,
    group_b_ptrs,
    group_c_ptrs,
    group_gemm_sizes,
    ...
    for g in range(group_size):
        ...
        if tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles:
            ...
            while(tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles):
                ...
                for kk in range(0, tl.cdiv(k, BLOCK_SIZE_K)):
                    a = a_desc.load([offs_am, kk * BLOCK_SIZE_K])
                    b = b_desc.load([kk * BLOCK_SIZE_K, offs_bn])
...
def group_gemm_tma_no_transpose_fn(group_A, group_B):
    ...
    grid = lambda META: (META['NUM_SM'], )
    grouped_matmul_tma_no_transpose_kernel[grid](d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, group_size,
                                    FP8=torch.float8_e4m3fn == group_A[0].dtype, NUM_SM=num_sms())
    return group_C
...
prof.export_chrome_trace('torch_matmul_for_loop.json')
...
# warmup
_ = group_gemm_tma_fn(group_A, group_B_T)
...
if supports_tma():
    ...
    prof1.export_chrome_trace('group_matmul_tma_kernel_without_autotune.json')
    # tri_tma_out_no_transpose = group_gemm_tma_no_transpose_fn(group_A, group_B)
    for i in range(group_size):
        assert torch.allclose(ref_out[i], tri_tma_out[i], atol=1e-2, rtol=1e-2)
        # assert torch.allclose(ref_out[i], tri_tma_out_no_transpose[i], atol=1e-2, rtol=1e-2)
...
def triton_tma_no_transpose_perf_fn(a_ptrs, b_ptrs, c_ptrs, sizes, lds, group_size, dtype):
    grid = lambda META: (META['NUM_SM'], )
    grouped_matmul_tma_no_transpose_kernel[grid](a_ptrs, b_ptrs, c_ptrs, sizes, lds, group_size, FP8=torch.float8_e4m3fn == dtype,
                                    NUM_SM=num_sms())
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_vals=['cublas', 'triton'] + (['triton-tma', 'triton-tma-no-T'] if supports_tma() else []),
        # label name for the lines
        line_names=["cuBLAS", "Triton"] + (['Triton + TMA', 'Triton-TMA-no-T'] if supports_tma() else []),
        # line styles
        styles=[('green', '-'), ('blue', '-')] + ([('red', '-'), ('yellow', '-')] if supports_tma() else []),
...
def benchmark_square_matrices(N, provider):
    ...
    if provider == 'triton-tma-no-T':
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: triton_tma_no_transpose_perf_fn(d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, group_size, dtype=torch.
                                       float16), quantiles=quantiles)
...
# benchmark_square_matrices.run(show_plots=True, print_data=True, save_path=".")
# benchmark_batches.run(show_plots=True, print_data=True, save_path=".")